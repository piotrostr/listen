...
pub async fn run_server(
    ...
    let adapter_state = if with_adapter {
        let redis_url = redis_url.expect("Redis URL required when adapter is enabled");
        Some(
            AdapterBuilder::new(redis_url)
                .with_data_routes()
                .with_chat_routes()
                .with_websocket_routes()
                .with_route_prefix("/adapter")
                .build()
                .await
                .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?,
        )
        ...
        if let Some(adapter_state) = adapter_state.as_ref() {
            app = app.configure(|config| {
                AdapterBuilder::configure_routes(
                    ...
                    true,  // with_chat
                    true,  // with_websocket
                    false, // don't add health check since we have our own
                    "/adapter",
                );
            });
        }
