...
export function aggregatePortfolioItems(items: PortfolioItem[]): AggregatedPortfolioItem[] {
  const aggregationMap = new Map<string, AggregatedPortfolioItem>();

  items.forEach((item) => {
    // Get the canonical symbol (e.g., WETH -> ETH)
    const canonicalSymbol = SYMBOL_AGGREGATION_MAP[item.symbol.toUpperCase()] || item.symbol;
    ...
    if (SYMBOL_AGGREGATION_MAP[item.symbol.toUpperCase()]) {
      ...
      if (aggregationMap.has(key)) {
        ...
        const totalValue = existing.originalItems.reduce((sum, i) => sum + (i.price * i.amount), 0);
        const totalAmount = existing.originalItems.reduce((sum, i) => sum + i.amount, 0);
        existing.price = totalAmount > 0 ? totalValue / totalAmount : 0;
        
        // Recalculate weighted average price change
        existing.priceChange24h = totalAmount > 0 
          ? existing.originalItems.reduce((sum, i) => sum + (i.priceChange24h * i.amount), 0) / totalAmount
          : 0;
      } else {
