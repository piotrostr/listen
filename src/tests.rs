...
use crate::{buyer::check_top_holders, constants, provider::Provider, tx_parser, util::env};
...
fn test_parse_mint_acc() {
    ...
    let _ =
        Mint::unpack(bs58::decode(data).into_vec().unwrap().as_slice()).expect("unpack mint data");
}
...
async fn test_gets_top_holders() {
    ...
    let (_, ok) = check_top_holders(&mint, &Provider::new(env("RPC_URL").to_string()))
        .await
        .unwrap();
    assert!(ok);
}
