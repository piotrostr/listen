...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match app.command {
        ...
        Command::PriorityFee {} => {
            println!(
                ...
                provider.rpc_client.get_recent_prioritization_fees(
                    vec![Pubkey::from_str(constants::RAYDIUM_LIQUIDITY_POOL_V4_PUBKEY).unwrap()]
                        .as_slice()
                )
            );
        }
        Command::ListenPools { snipe } => {
            ...
            let wallet = Keypair::read_from_file(must_get_env("HOME") + "/.config/solana/id.json")?;
            let listener = tokio::spawn(async move {
                while let Ok(log) = recv.recv() {
                    ...
                    let new_pool_info =
                        tx_parser::parse_new_pool(&provider.get_tx(&log.value.signature).unwrap())
                            .unwrap();
        ...
        Command::Swap {
            ...
            if dex.unwrap_or("".to_string()) == "raydium" {
                ...
                let amount_specified = if amount.is_some() {
                    ...
                    provider.get_spl_balance(&wallet.pubkey(), &input_token_mint)?
                };
